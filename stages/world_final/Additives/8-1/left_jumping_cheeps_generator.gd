...
@export var speed_min := Vector2i(5 * 50, -8 * 50)
@export var speed_max := Vector2i(11 * 50, -5 * 50)
...
func _time() -> void:
	...
	if Thunder.rng.get_randi_range(0, random_pool) == random_equals:
		...
		var to_pos = Vector2(Thunder.view.border.position.x + 160, Thunder.view.border.end.y) + Vector2(Thunder.rng.get_randi_range(-100, 0), 0)
